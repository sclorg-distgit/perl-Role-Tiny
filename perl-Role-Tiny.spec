%{?scl:%scl_package perl-Role-Tiny}

Name:           %{?scl_prefix}perl-Role-Tiny
Version:        2.001004
Release:        2%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(Class::Method::Modifiers) >= 1.05
BuildRequires:  %{?scl_prefix}perl(Exporter)
BuildRequires:  %{?scl_prefix}perl(mro)
...
Requires:       %{?scl_prefix}perl(Class::Method::Modifiers) >= 1.05
Requires:       %{?scl_prefix}perl(mro)

# perl-Role-Tiny was split from perl-Moo
Conflicts:      %{?scl_prefix}perl-Moo < 0.009014
...
%check
%{?scl:scl enable %{scl} '}%{make_build} test%{?scl:'}
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 2.001004-2
- SCL
